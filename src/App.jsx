...
export default function App() {
  ...
  >
    ...
    <p style={{ color: styles.textMuted }} className="text-xs leading-relaxed">Aralashgan professional testlarni topshirish xonasi.</p>
              ...
              <motion.div style={{ backgroundColor: styles.cardBg, border: styles.cardBorder, boxShadow: '0 2px 4px rgba(255, 255, 255, 0.4)'  }} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="max-w-md w-full mx-auto p-8 rounded-2xl shadow-xl">
                <form onSubmit={role === 'teacher' ? (savedTeacher ? handleTeacherLogin : handleTeacherRegister) : handleStudentLogin} className="space-y-4">
                  ...
                  <button
                    type="submit"
                    style={{
                      backgroundColor: darkMode ? '#ffffff' : '#0f172a', // Darkda oq, Lightda qora fon
                      color: darkMode ? '#0f172a' : '#ffffff',           // Darkda qora, Lightda oq yozuv
            ...
            {role && isLoggedIn && quizState === 'list' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 w-full">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="lg:col-span-2 space-y-4">
                    <h2 style={{ color: styles.text }} className="text-lg font-black flex items-center gap-2"><Award className="text-indigo-500" size={18} /> Mavjud Imtihonlar ({quizzes.length})</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {quizzes.map(quiz => (
                        <motion.div key={quiz.id} style={{ backgroundColor: styles.cardBg, border: styles.cardBorder }} className="p-5 rounded-2xl shadow-sm flex flex-col justify-between transition-all">
                          <div>
                          ...
                          <div style={{ borderTop: styles.cardBorder }} className="flex items-center justify-between pt-3">
                            <span style={{ color: styles.textMuted }} className="text-[11px] font-medium">{quiz.questions.length} ta savol</span>
                            {role === 'student' ? (
                              <button onClick={() => handleStartQuiz(quiz)} className="flex items-center gap-1 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold px-3 py-2 rounded-xl transition-all shadow-sm"><Play size={10} fill="white" /> Boshlash</button>
                            ) : (
                              <button onClick={() => setQuizzes(quizzes.filter(q => q.id !== quiz.id))} className="p-2 text-red-500 hover:bg-red-50 rounded-xl transition-all"><Trash2 size={15} /></button>
                            )}
                          </div>
                        </motion.div>
                      ))}
                  ...
                  {role === 'teacher' && (
                    <div style={{ backgroundColor: styles.cardBg, border: styles.cardBorder }} className="p-5 rounded-2xl shadow-sm h-fit max-h-[75vh] overflow-y-auto">
                      <h3 style={{ color: styles.text }} className="font-bold text-base mb-3 flex items-center gap-1.5"><Plus className="text-indigo-500" size={18} /> Yangi Test Yaratish</h3>
                      <form onSubmit={handleCreateQuiz} className="space-y-3.5">
                        <input type="text" required value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="Fan nomi" style={{ backgroundColor: styles.inputBg, color: styles.text, border: styles.cardBorder }} className="w-full px-3 py-2 rounded-xl text-xs focus:outline-none" />
                        <textarea required value={newDesc} onChange={e => setNewDesc(e.target.value)} placeholder="Tavsif yoki izoh" rows="2" style={{ backgroundColor: styles.inputBg, color: styles.text, border: styles.cardBorder }} className="w-full px-3 py-2 rounded-xl text-xs focus:outline-none"></textarea>
                        <input type="number" value={newDuration} onChange={e => setNewDuration(e.target.value)} placeholder="Vaqt (daqiqa)" style={{ backgroundColor: styles.inputBg, color: styles.text, border: styles.cardBorder }} className="w-full px-3 py-2 rounded-xl text-xs focus:outline-none" />

                        <div style={{ borderTop: styles.cardBorder }} className="pt-3 space-y-3">
                          <div className="flex items-center justify-between"><label style={{ color: styles.textMuted }} className="text-[11px] font-bold uppercase">Savollar</label><button type="button" onClick={() => setNewQuestions([...newQuestions, { text: '', options: ['', '', '', ''], correctAnswer: 0 }])} className="text-xs font-bold text-indigo-500 hover:underline">+ Yangi Savol</button></div>
                          {newQuestions.map((q, qIdx) => (
                            <div key={qIdx} style={{ backgroundColor: styles.bg, border: styles.cardBorder }} className="p-3 rounded-xl space-y-2">
                              <input type="text" required placeholder={`Savol matni`} value={q.text} onChange={e => { const u = [...newQuestions]; u[qIdx].text = e.target.value; setNewQuestions(u); }} style={{ backgroundColor: styles.cardBg, color: styles.text, border: styles.cardBorder }} className="w-full px-2.5 py-1.5 rounded-lg text-xs" />
                              {q.options.map((opt, oIdx) => (
                                <div key={oIdx} style={{ backgroundColor: styles.cardBg, border: styles.cardBorder }} className="flex items-center gap-2 px-2 py-1 rounded-lg">
                                  <input type="radio" name={`correct-${qIdx}`} checked={q.correctAnswer === oIdx} onChange={() => { const u = [...newQuestions]; u[qIdx].correctAnswer = oIdx; setNewQuestions(u); }} />
                        ...
                        <button type="submit" className="w-full bg-indigo-600 text-white text-xs font-bold py-2.5 rounded-xl hover:bg-indigo-700 transition-all">Saqlash</button>
                      </form>
                    </div>
                  )}
                </div>
                ...
                {role === 'teacher' && (
                  <div style={{ backgroundColor: styles.cardBg, border: styles.cardBorder }} className="rounded-2xl p-5 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                      <h3 style={{ color: styles.text }} className="font-bold text-base flex items-center gap-1.5">Natijalar (Reyting)</h3>
                    </div>
                    {results.length === 0 ? (
                      <p style={{ color: styles.textMuted }} className="text-xs text-center py-4">Hozircha natijalar yo'q.</p>
                    ) : (
                      <div className="overflow-x-auto">
                        <table className="w-full text-left text-xs">
                          <tbody style={{ color: styles.text }}>
                            {results.map(res => (
                              <tr key={res.id} style={{ borderBottom: styles.cardBorder }}>
                                <td className="p-2.5 font-bold">{res.studentName}</td>
                                <td>{res.quizTitle}</td>
                                <td>{res.score}/{res.totalQuestions}</td>
                                <td><span className="px-2 py-0.5 rounded font-bold bg-emerald-100 text-emerald-700">{res.percent}%</span></td>
                  ...
                  {currentQuiz.questions.map((q, qIdx) => (
                    <div key={q.id} className="space-y-2.5">
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        {q.options.map((opt, oIdx) => (
                          <button key={oIdx} onClick={() => setAnswers({ ...answers, [qIdx]: oIdx })} style={{ backgroundColor: answers[qIdx] === oIdx ? 'rgba(79, 70, 229, 0.15)' : styles.bg, color: styles.text, border: answers[qIdx] === oIdx ? '2px solid #4f46e5' : styles.cardBorder }} className="w-full text-left px-4 py-2.5 rounded-xl text-xs transition-all">{opt}</button>
                        ))}
                      </div>
                    </div>
                  ))}
                  <button onClick={handleFinishQuiz} className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold py-3 rounded-xl shadow-md flex items-center justify-center gap-1.5 text-xs"><CheckCircle size={15} /> Yakunlash</button>
                </div>
              </motion.div>
            )}
            ...
            {quizState === 'result' && (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} style={{ backgroundColor: styles.cardBg, border: styles.cardBorder }} className="max-w-md w-full mx-auto p-8 rounded-2xl shadow-xl text-center">
                ...
                <button onClick={() => setQuizState('list')} className="w-full bg-indigo-600 text-white font-bold py-2.5 rounded-xl shadow-md hover:bg-indigo-700 transition-all text-xs">Bosh sahifaga qaytish</button>
              </motion.div>
            )}
